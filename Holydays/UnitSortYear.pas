...
ShowMessage('Не верный формат даты')
else
begin
...
begin
  Active:=False;
  SQL.Clear;
  SQL.Add('Select Код_праздника,Код_типа_праздника, Код_заказчика, Название, Место_проведения,Дата_провидения,Стоимость, Оплачен,Тип_праздника,Заказчик');
  SQL.Add('From tmp');
  SQL.Add('Where Дата_провидения Between #01/01/'+edt1.Text+'# AND #01/01/'+IntToStr(StrToInt(edt1.Text)+1)+'#');
  Open;
end;
FormSortYear.Close;
end;
